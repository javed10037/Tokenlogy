	 ...
	 if(fromAddress && toAddress){
	 	web3.eth.sendTransaction({from: fromAddress, to:toAddress, value: web3.utils.toWei('1', 'ether'),
	 	 gasLimit: 21000, gasPrice: 20000000000})
			...
			.on('error', function(err){
				res.send({status : 400, message : 'Insufficient balance',error : err });
			});
...
var sendEthToUser = function(req, res){
	 let fromAddress = req.body.fromAddress;
	 let toAddress = req.body.toAddress;
	 let amount = req.body.amount;
	 let userId = req.body.userId;
	 let password = req.body.password;
	  ...
	  User.findOne({ _id: userId},{}, function (err, data) {
                ...
                if (err) {
                    ...
                    if(data){
            		 // web3.eth.personal.unlockAccount(fromAddress,password, 15000);  unlock by password
            		 web3.eth.personal.unlockAccount(toAddress,'kunvar@yopmail.com', 15000);
            		 console.log('**isAccountUnlock::',web3.eth.personal.unlockAccount(fromAddress,data.email, 15000));
...
var savedICOtokenIntoDB = function(req, res){
	console.log('*****api hitiing***** savedICOtokenIntoDB',req.body.icoId);
		...
		let ICOOBJ = {
				...
				whitePaper : whitePaper
						...
						ICO.create(ICOOBJ, function(err, ico){
							...
							if(ico){
								console.log('icos saved successfully:::');
								PublicApi.deleteCrowdsale(icoId,req,res,function(err,result)
								{
									console.log('**About to delete***')
									if(err){
										console.log('**About to delete err***')
										return res.status(HttpStatus.BAD_REQUEST).send({data: ico, status: HttpStatus.BAD_REQUEST});
									}
									else{
										console.log('**About to delete success***')
...
var getTokenByUserId = function(req, res){
		let userId = req.body.userId;
		if(userId){
			let condition = { $and: [ { userId: userId }, { isDeleted: false} ] };
			ICO.find(condition,{}).populate('userId',{}).exec(function(err,token){
				...
				if(token.length){
					...
					let counter=0;
					let result = [];
					Async.forEachLimit(token,1,(element,next)=>{
						 counter++;
						 console.log(counter);
	                     if(counter < token.length)
	                     {
	                     	let crowdsaleCounter = 0;
   	                     	let totalsupply = 0;
	                     	Async.forEachLimit(element.crowdsale,1,(crowdsalelement,next)=>{
	                     		crowdsaleCounter++;

	                     		totalsupply = totalsupply + parseInt(crowdsalelement.supply);
	                     		// console.log('(((((element))))))',totalsupply);

	                     		if(crowdsaleCounter < element.crowdsale.length){
	                     			next();
	                     		}else{
	                     			element.tokenSupply = totalsupply;
	                     			result.push(element);
	                     		}
	                     	});
...
let tokenId = req.body.tokenId;
	...
	res.json({message : "Please enter the valid id ",status : 400})
	  ...
	  res.json({message : "ICO has been deleted successfully!.",status : 200})
	} else{
	   res.json({message : "Please enter the valid id ",status : 400})
	 }
	})
...
exports.sendEthToUser = sendEthToUser;
